...
この文書は、別端末・別セッションから Creator Stack Lab / Creator Stack Observatory の作業を再開するための運用上の正本です。製品境界の定義は [`PRODUCT_BOUNDARY.md`](./PRODUCT_BOUNDARY.md) に集約しています。このファイルには現在状態、実装構造、検証済み事実、配備境界、次の作業を記録します。状態が変わったら、このファイルを更新してください。

## 2026-07-23 コピー表現の整理

UI、カタログ要約、Evidence Passport、My Stack、Observation、README、製品境界、引き継ぎ文書の表現を横断整理した。対象、条件、状態、次の行動を直接述べる構成に統一。否定を前置きして結論を後出しする対比構文を撤去した。

適合判定、Source状態、Observationロジック、優先順位、URL、保存境界、配備境界は変更していない。文言を固定していたrendered HTML testとMy Stack testは、新しい表示内容へ追従させた。表現原則は `docs/PRODUCT_BOUNDARY.md` に記録済み。

| Gate | 結果 |
|---|---|
| Targeted copy scan | README、docs、app、components、data、lib、workerを対象に21種類の対比・逆接パターンを検索。該当0件 |
| Production build / tests | `npm test` 成功。rendered HTML 6 + domain 27 = **33 / 33成功** |
| Static gates | `npm run lint`、`npx tsc --noEmit`、`git diff --check` 成功 |
| Deployment | 未実施。Sites source、saved version、productionは変更していない |

## 2026-07-23 リモート同期・開発再開監査 / 監修AI向け要約

GitHub `origin` を `fetch --prune` した後、`git pull --ff-only origin master` を実行した。監査対象のfeature / source checkpointではlocal `master`、`origin/master`、`HEAD` がすべて `d42ec4963c2edd980a872e161fd88ffb99ef2165` で一致し、ahead 0 / behind 0、tracked worktreeはcleanだった。GitHub上の共同作業正本はpublic repository `YuShimoji/CreatorStackLab`、default branchは `master`。取り込むべきリモート差分はなく、My Stack変更要約 v0はsource checkpointへ既に含まれている。この監査を記録するdocs-only closeout commitは `d42ec49` の後継になる。再開時は最新の `origin/master` を厳密なGitHub HEADとして用いる。

**結論:** ローカルは次の実装・acceptance作業を開始できる。source/build/testの回帰は検出していない。未完了項目は、My Stack新UIの実ブラウザacceptance、GitHub / Sites / productionのSHA一致、owner表示、production依存のhigh advisory 2件に対する配備判断である。`d42ec49` の証拠範囲はsource実装と自動検証まで。production反映とowner受入は未確認。
...
| Static gates | `npm run lint`、`npx tsc --noEmit`、`git diff --check` 成功 |
| Local-only state | `.serena/`、`.wrangler/`、`dist/`、`node_modules/`、`tsconfig.tsbuildinfo` はignoredのまま保存。広範削除や別端末への転送対象にしていない |
...
vinext dev logの初回compile時間は非常に大きな値で表示された。実リクエスト時間、HTTP結果、build、testsはすべて成功。Node 24 / vinextのログ表示上の問題として、依存保守時に再確認する。

### GitHub / Sites / production のlive境界

2026-07-23にSites control planeをread-only確認した。既存Siteは `active`、access mode `custom`、許可ユーザー1名、workspace / tenant group 0、latest saved versionはVersion 8で、そのsource commitは `eca336d15d48cc2b08533a79ff2bb17d7f8a79da`。GitHubのfeature / source checkpoint `d42ec49` はSites latest saved sourceより2 commits先である。この後のhandoff更新はdocs-onlyで、runtime source parityを成立させない。匿名でproduction `/` と `/api/observations/snapshot` を取得するとどちらもHTTP 401であり、owner-only入口は維持されている。

control planeのreadbackには、現在liveなproduction deploymentのversionを特定できる情報がなかった。過去の正本には最後のfeature deploymentとしてVersion 6 / `278f507` が記録されている。2026-07-23時点のlive SHAは未確認。owner表示、run APIの匿名401、production body、GitHub / Sites / deployment archiveの同一SHAは次のacceptance / deployment担当が確認する。

### 依存セキュリティの現在値

2026-07-23のregistry readbackでは、`npm audit --omit=dev` は **moderate 1 / high 2 / critical 0**。`next@16.2.6` 経由の `postcss <8.5.10` と `sharp <0.35.0` が対象で、`npm audit fix --force` はNext 9.3.3へのbreaking downgradeを提示するため実行していない。全依存では **low 1 / moderate 5 / high 11 / critical 0** で、Babel、brace-expansion、esbuild、fast-uri、js-yaml、PostCSS、sharp、undici、Vite、ws等を含む。audit commandは `--audit-level=critical` の条件を満たして終了コード0となった。上記advisoryは残っている。

Browser acceptanceは先行できる。private checkpoint deployment前に、production high advisoryへの対応方針を監修役が次のどちらかで明示する。
...
private operational betaを10段階とした概算は **6 / 10**（`██████░░░░`）。Catalog / Repository境界、4公式SourceのObservation、D1履歴、Evidence Passport、Today / Changes / Compare / My Stack、変更要約source、自動検証は存在する。残る主要gapは、My Stack変更要約の実ブラウザ・owner snapshot受入、source / saved version / live deploymentの同一SHA、security maintenance、実在pendingを扱うReview Queue運用、定期観測前の復旧・保持・監査契約である。評価対象は、製品境界内のowner-only private beta。一般公開サービスの完成度は評価していない。
...
| GitHub / Sites / live SHAを一致させる | 「sourceは新しいがproductionは古い」状態を解消 | 既存Siteのみ、短期credentialのみ、同一commit archive、access custom、user 1、group 0 | GitHub feature checkpoint `d42ec49` + 後続docs、Sites latest saved `eca336d`。live versionは未特定 | 配備権限を持つ担当 | acceptanceとsecurity gate後、最新 `origin/master` の同一sourceをSitesへpush、save、private deploy、SHA readback |
| owner / anonymous production acceptance | private runtimeの機能・認証境界を証明 | owner body、My Stack、snapshot / run、匿名 `/` / APIs 401、console、responsive | 匿名 `/` とsnapshot APIの401はlive再確認。owner bodyとrun APIは未確認 | owner sessionを持つQA担当 | deployment後に実ブラウザで確認し、結果を本書へ反映 |
| カタログ鮮度deadlineに追従する | 期限超過前に制作判断の公式根拠と未知を再確認する | Source Admission Policy、公式Source、確認日、条件 / 未知 / 変更履歴の分離 | `setup-gomixer-garageband` が2026-07-30、その後2026-08-16に4件が再確認期限。未実施 | Evidence / catalog担当 | 7月30日対象を最優先にread-only予備調査し、書き換えはdeploymentと別スライスにする |
| 実在pendingに対するReview Queue運用 | terms等の意味を人が判断し、その履歴を追跡可能にする | 実在 `review_pending`、approve / reject / supersede、actor / timestamp / provenance、再実行の冪等性 | データ層とqueueはある。operator UIは未実装 | 次期feature担当 + 判断owner | 実在pendingまたは正本fixtureを使い、必要最小限のvertical sliceを定義 |
...
| H3 — Observation reliability beta | 4既存Sourceについて取得・304・timeout・malformed・stale・last success・retryを運用可能にする | bounded retry、per-source health、idempotent run、保持期間、export / restore drill、fetch failureとservice statusの分離 |
...
監修役AIの推奨順序は `H1 Browser acceptance → security decision → private deployment → H2 Review → H3 reliability`。H4以降は、H1〜H3で得た実利用証拠に基づき再評価する。`setup-gomixer-garageband` のread-only予備調査は並行で開始できる。2026-07-30の鮮度deadlineを守るための先行確認である。

## 2026-07-21 My Stack変更要約 v0 source checkpoint

**My Stack単位の変更要約 v0** のsource実装と全自動検証を完了した。この文書と同じcommitをGitHub `origin/master` の共同作業正本とする。Sites source repository、owner-only production、Browser QAはこのcheckpointで更新・再検証していない。GitHub sourceの完了とproduction配備は別の証拠ゲートである。
...
- `previousVisit` より前の変更はbaselineとして扱う。`previousVisit` 不明時も同じ扱いとする。
- baseline、unchanged、fetch failure、Source未接続を個別の状態として保持する。
...
`npm audit --omit=dev` の修正候補はNext 9.3.3へのmajor downgradeを提示するため採用しない。全体auditには `@cloudflare/vite-plugin` 1.45.1、Vite 8.1.5等の非major候補も含まれる。独立した依存保守スライスで互換性を確認する。`npm audit fix`、force fix、override、依存version変更は行っていない。
...
2. GitHub source、Sites `main`、productionのSHAを個別に取得し、差分を記録する。
...
| Git worktree | 監査開始時はtracked差分なし。端末固有の `.serena/` は保存したまま `.git/info/exclude` でlocal除外 |

### 残作業と証拠境界

| 目的 | 影響 | 要件 | 状態 | Owner | 次の一手 |
|---|---|---|---|---|---|
| production依存の脆弱性を解消する | `npm audit --omit=dev` はmoderate 2件。`next@16.2.6` 同梱の `postcss@8.4.31` が `<8.5.10` のadvisory対象 | Next / vinext互換性とbuild・33 tests・owner-only回帰を維持する。監査が提示するNext 9.3.3への大幅downgradeは採用しない | 未解消。build・test・dev serverは成功。production security maintenanceが残る | 次回の依存保守担当（監修AIが割当） | vinext / Next側の互換修正版または安全なoverride可否を調査し、独立スライスで更新・全gate再検証 |
...
GitHub `origin/master` の最新 `HEAD` を共同作業の正本とします。Sites source repositoryの `main` は配備用mirrorです。2026-07-21のGitHub source checkpointは未同期です。次の配備前にGitHub、Sites、productionのSHAとowner-only境界をlive取得してください。
...
Creator Stack Observatoryは、公式情報と外部証拠を、出典、適用条件、鮮度、変更履歴、未知とともに編成する制作判断の観測所です。対象外の領域は、実機検証ラボ、ガジェットブログ、購買ランキング、自由投稿コミュニティ、一般障害監視、広告・アフィリエイト媒体です。
...
- 情報源の取得失敗は `fetch_failed` として表示する。製品・サービスの稼働状態は別の観測対象とする。
- 運営者実機試験の未実施を「証拠なし」と表示しない。
- Source追加は、制作判断への必要性と維持価値で決める。網羅性は目標に含めない。
...
Epistemic Statusには、`officially_stated`、`observed_current`、`conditional`、`unknown`、`conflicted`、`stale` の6つの離散状態を使います。
...
|---|---|---|---|
| `twitch-status` | `setup-ag03-ipad-usbc` | Statuspage JSON | 公式サービス状態を観測。ローカル構成適合は対象外 |
...
D1 binding名は `DB` です。既存テーブルは `sources`、`observations`、`change_events`、`update_runs`、`review_queue` です。Evidence Passport v0のスキーマmigration追加数は0件です。

Snapshot履歴は直近Observationを `baseline`、`unchanged`、`changed`、`fetch_failed`、`review_pending` へ要約します。Product Surfaceには判断に必要な範囲を表示します。
...
Todayの各公式Sourceカードから、対応するEvidence Passportへ移動できます。Source未接続対象の詳細も表示し、公式観測の状態を未接続と明示します。

## My Stackの現在境界

- 保存先はlocalStorageで、このブラウザ・この端末だけに存在する。
- アカウント同期、クラウド同期、メール・push通知はない。
- Providerのready前は読込中表示を行う。件数と状態はready後に確定する。
- `previousVisit` はlocalStorageの既存値を読み取ってから現在時刻で上書きし、変更要約の比較基準に使う。
- 保存entityだけを対象に、Source単位の状態を失わずentity単位の変更要約を表示する。
- snapshotのloading / error時は状態を確認待ちとして表示し、Evidence Passportの根拠と未知へ戻れる。
...
外部Sourceの値と鮮度は時間で変化します。再開時はownerの `/status` とEvidence Passportで現在値を確認してください。初回client hydration直後の `実観測を読み込み中` は、Observation Snapshotの取得中を示します。状態はまだ確定していません。

GitHub source checkpointでは2026-07-21にproduction build、33 / 33 tests、lint、TypeScript、diff checkまで成功しています。証拠範囲はsourceと自動検証です。新UIのBrowser QA、Sites同期、owner / anonymous production確認は未実施です。
...
- ready前、My Stack空、previousVisit不明、snapshot loading / errorを確認する。各状態が固有の表示を持つことを検証する。
...
> `docs/PROJECT_HANDOFF.md` と `docs/PRODUCT_BOUNDARY.md`、GitHub `origin/master` の最新HEADを正本として、まずMy Stack変更要約 v0のBrowser acceptanceを行ってください。GitHub HEAD、Sites latest saved source、production version、access policyをlive取得し、それぞれの値と差分を記録してください。checkout所有のnpm / nodeプロセスを確認してnpm操作を直列化し、`npm ci`、`npm test`（33 tests想定。実出力を正本とする）、lint、TypeScript、diff checkを再確認してください。1440px・390pxでready、空、初回baseline、VOICEVOX複数Source、Catalog-only、snapshot error、Evidence Passport / history遷移、console 0、横溢れなし、既存workflow回帰を確認してください。次にproduction auditのmoderate 1 / high 2をlive再確認し、互換修正または期限付きrisk acceptanceを監修判断として記録してください。security gate通過後、GitHub HEADと同一SHAをSites `main`へ同期し、既存Siteへprivate checkpoint deploymentしてください。owner成功、匿名 `/` / snapshot API / run API 401、許可ユーザー1名、group 0、GitHub / Sites / archive / live source parity、clean treeまで確認してください。変更範囲はBrowser acceptance、security decision、private checkpoint deploymentです。新規Site、Source、DB / 認証 / API変更、Review Queue操作、定期実行、cloud sync、通知、一般公開、広告は対象外です。Next 9 downgrade、force fix、検証なしoverrideは禁止です。
