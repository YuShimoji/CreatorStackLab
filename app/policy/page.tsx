...
const evidenceRows = [
  ...
  ["公式情報確認済み・実機未検証", "メーカー、開発元、公式規約、公式マニュアルで確認済み。当サイトでの物理構成再現は未実施。"],
...
export default function PolicyPage() {
  return (
    <div className="section-shell page-shell policy-page">
      <header className="page-intro"><p className="section-kicker">VERIFICATION POLICY</p><h1>検証方針・運営ポリシー</h1><p>確認済みの範囲、未確認事項、判断根拠を明示します。</p></header>
      <section className="policy-section"><h2>判定状態</h2><div className="policy-status-grid"><div><VerdictBadge verdict="適合" /><p>対象用途と構成について、確認範囲では追加条件なしに成立。</p></div><div><VerdictBadge verdict="条件付き適合" /><p>プラン、クレジット、接続、給電、設定等の条件を満たせば成立。</p></div><div><VerdictBadge verdict="不適合" /><p>対象経路では成立しない一次情報または再現結果がある。</p></div><div><VerdictBadge verdict="未確認" /><p>根拠不足。判定を保留している状態。</p></div></div></section>
      ...
      <section className="policy-section"><h2>実機検証プロトコル</h2><ol className="protocol-list"><li><span>01</span><div><h3>構成を固定する</h3><p>端末、OS、アプリ版、オーディオI/F、マイク、ケーブル、アダプター、給電を記録します。</p></div></li><li><span>02</span><div><h3>経路を段階確認する</h3><p>入力認識、チャンネル、ステレオ／モノラル、モニター、ループバック、録音・配信先を順に確認します。</p></div></li><li><span>03</span><div><h3>条件と失敗を保存する</h3><p>成立した設定、失敗した設定、回避策、再現手順を記録します。</p></div></li><li><span>04</span><div><h3>時点を明示する</h3><p>確認日とバージョンを保存し、更新後は再検証前の結果を「現行確認済み」と扱いません。</p></div></li></ol></section>
      <section className="policy-section two-policy-columns"><div><h2>出典・訂正</h2><ul><li>メーカー、開発元、公式規約、公式マニュアルを優先します。</li><li>原文URLと確認日を根拠として保存します。</li><li>規約本文から、判断に必要な要点を要約します。</li><li>誤りを確認した場合は、判定・出典・変更履歴を同時に訂正します。</li></ul></div><div><h2>広告・提供・スポンサー</h2><ul><li>初版には広告、アフィリエイト、スポンサー表示はありません。</li><li>導入時は公式情報・公式販売・小売・アフィリエイトをデータ上で区別します。</li><li>提供品やスポンサーがある場合は、対象レコードと方針ページの双方で明示します。</li><li>収益の有無で判定順位や結論を変えません。</li></ul></div></section>
      <section className="legal-note"><h2>確認範囲</h2><p>本サイトは公式情報と観測記録を整理します。法的助言、契約解釈の保証、機材メーカーによる動作保証は対象外です。重要な公開・納品・購入の前に、最新の原文と提供元の回答を確認してください。</p></section>
    </div>
  );
}
